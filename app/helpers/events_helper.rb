module EventsHelper
  def get_flash_for_user
    ...
    @requests = Request.where(user_id: session[:current_user_id], notification_type: 'attendance', status: %w(approved rejected))
    if @requests.count > 0
      @requests.each do |request|
    ...
    if  @requests.count > 0
      @requests.each do |request|
        flashmessage += "You have a new survey to take. #{link_to 'Click here to take the survey now! ', new_survey_path(event_id: request.event_id)}<br />"
      end
    end
