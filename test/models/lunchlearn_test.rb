...
class LunchlearnTest < ActiveSupport::TestCase
  ...
  test 'should save event' do
    ...
    @event = Lunchlearn.new( title: Faker::Name.title, description: Faker::Hacker.say_something_smart, lunch_date: random_start.to_date, lunch_time: random_start.to_time, meeting_phone_number: "", access_code: "", has_GoToMeeting: false, go_to_meeting_url: "", end_time: random_end.to_time)
    assert @event.save
  end
